...
import { 
  TrendingUp, Smile, Bot, BrainCircuit, Activity, 
  CheckSquare, Users, Calendar, ArrowUpRight
} from 'lucide-react';
...
const ExecutiveBriefing = ({ hasFinanceAccess }: { hasFinanceAccess: boolean }) => {
  const financial = nexusStore.getFinancialSummary();
  const tasks = nexusStore.tasks.filter(t => t.status !== 'Completed').length;
  const currentUser = nexusStore.currentUser;

  return (
    <div className="bg-slate-950 rounded-[3rem] p-10 md:p-14 text-white relative overflow-hidden mb-12 shadow-2xl border border-white/10 group">
      ...
      <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-12">
        <div className="max-w-2xl">
          <div className="flex items-center gap-5 mb-8">
            <div className="w-16 h-16 bg-[#00ff9d] rounded-2xl flex items-center justify-center shadow-2xl shadow-indigo-500/50 animate-float">
              <BrainCircuit size={36} className="text-slate-950" />
            </div>
            <div>
...
export const Dashboard = () => {
  const [dataState, setDataState] = useState({
    financial: nexusStore.getFinancialSummary(),
    nps: nexusStore.getNPS(),
    tasksCount: nexusStore.tasks.filter(t => t.status !== 'Completed').length,
    employeeCount: nexusStore.employees.length,
    eventsCount: nexusStore.cateringEvents.filter(e => e.status === 'Confirmed').length
  });
    ...
    <div className="space-y-10 animate-in fade-in pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        ...
        <div className="flex items-center gap-4 bg-white/50 backdrop-blur-md px-6 py-3 rounded-3xl shadow-sm border border-white/80">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs font-black text-slate-700 uppercase tracking-tighter">Sync Active</span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {hasFinanceAccess || hasPartialSalesAccess ? (
          <Card title="Revenue Flow" value={`₦${(dataState.financial.revenue / 1000000).toFixed(1)}M`} sub="+12.5% vs LW" icon={TrendingUp} gradient="from-indigo-600 to-indigo-800" />
        ) : (
          <Card title="Pending Tasks" value={dataState.tasksCount.toString()} sub="Action Required" icon={CheckSquare} gradient="from-indigo-600 to-indigo-800" />
        )}

        {hasFinanceAccess ? (
          <Card title="Cash Liquidity" value={`₦${(dataState.financial.cash / 1000000).toFixed(1)}M`} sub="Live Balance" icon={Activity} gradient="from-emerald-500 to-teal-700" />
        ) : isHR ? (
          <Card title="Headcount" value={dataState.employeeCount.toString()} sub="Active Staff" icon={Users} gradient="from-emerald-500 to-teal-700" />
        ) : (
          <Card title="Events Active" value={dataState.eventsCount.toString()} sub="In Pipeline" icon={Calendar} gradient="from-emerald-500 to-teal-700" />
        )}

        <Card title="Sentiment Index" value={dataState.nps.toString()} sub="+5 pts Up" icon={Smile} gradient="from-fuchsia-600 to-pink-700" />
