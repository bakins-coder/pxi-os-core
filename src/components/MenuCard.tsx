    ...
    return (
        ...
        >
                ...
                {showInfo && (
                    ...
                    >
                        <div className="flex justify-between items-start mb-2 md:mb-2">
                            <h4 className="text-sm md:text-sm font-black uppercase tracking-tight text-slate-900 leading-tight break-words pr-2">{item.name}</h4>
                        ...
                        <div className="flex-1 overflow-y-auto no-scrollbar">
                            <p className="text-[11px] md:text-xs text-slate-600 font-medium leading-relaxed whitespace-pre-wrap">
                                {item.description || 'No detailed description available for this item.'}
                            </p>
                        </div>
                        <div className="pt-2 md:pt-2 mt-2 border-t border-slate-100 text-[9px] md:text-[8px] text-slate-400 font-bold uppercase tracking-widest text-center">
            ...
            <div className="p-3 md:p-4 flex-1 flex flex-col">
                ...
                <p
                    onClick={(e) => { e.stopPropagation(); setShowInfo(true); }}
                    className="text-[9px] md:text-[9px] text-slate-500 font-bold uppercase mb-3 leading-relaxed hover:text-indigo-600 transition-colors whitespace-pre-wrap break-words"
                >
                    {item.description}
                </p>
                ...
                <div onClick={(e) => e.stopPropagation()} className="mt-auto space-y-1 p-2 md:p-3 bg-slate-50 rounded-lg md:rounded-xl border border-slate-100">
                    <div className="flex justify-between items-center mb-1 md:mb-0">
                        ...
                        <input
                            type="number"
                            className="w-12 md:w-16 bg-white border border-slate-200 rounded-md md:rounded-lg py-0.5 md:py-0.5 text-center text-[10px] md:text-[10px] font-black text-slate-950 outline-none focus:border-indigo-500 shadow-sm"
                            value={qty}
                            max={999}
                    ...
                    <div className="flex items-center gap-1.5 md:gap-2">
                        <div className="p-1.5 bg-white border border-slate-200 rounded-md shadow-sm active:scale-95 transition-transform cursor-pointer" onClick={() => updateQty(item.id, Math.max(0, qty - 5))}>
                        ...
                        <input
                            ...
                            step="5"
                            className="flex-1 accent-indigo-600 h-1.5 md:h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer"
                            value={qty}
                            onChange={(e) => updateQty(item.id, parseInt(e.target.value))}
                        />
                        <div className="p-1.5 bg-white border border-slate-200 rounded-md shadow-sm active:scale-95 transition-transform cursor-pointer" onClick={() => updateQty(item.id, qty + 5)}>
