...
const MarketingStudio = () => {
   ...
   const { marketingPosts: posts, socialInteractions: interactions, addMarketingPost } = useDataStore();
      ...
      <div className="space-y-6">
         <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
            ...
            <button
               ...
               disabled={strictMode}
               className={`${strictMode ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-[#00ff9d] text-slate-950 shadow-sm'} px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all`}
            >
               <Sparkles size={16} /> {strictMode ? 'AI Locked' : 'AI Generator'}
            </button>
         </div>
         ...
         {strictMode && (
            <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl flex items-center gap-3">
               ...
               <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 italic">Semantic protocols are currently suspended by global command.</span>
            </div>
         )}
...
export const Automation = () => {
   ...
   const handleRunWorkflow = async (workflow: Workflow) => {
      ...
      await executeAgentWorkflow(workflow.id, workflow.agentName, workflow.agentRole, "Manual test context from DataStore.");
      setRunningId(null);
   };
               ...
               {workflows.map(wf => (
                  <div key={wf.id} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-xl flex flex-col justify-between group hover:border-[#00ff9d] transition-all">
                     ...
                     <button
                        ...
                        disabled={runningId === wf.id || strictMode}
                        className={`w-full py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl flex items-center justify-center gap-3 transition-all active:scale-95 ${strictMode ? 'bg-slate-100 text-slate-400 cursor-not-allowed shadow-none' : 'bg-slate-950 text-white'
                           }`}
                     >
                        {runningId === wf.id ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin"></div> : strictMode ? <Edit3 size={14} /> : <Play size={14} />}
                        {runningId === wf.id ? 'Agent Active' : strictMode ? 'Manual Override Active' : 'Trigger Execution'}
                     </button>
                  </div>
               ))}
            </div>
