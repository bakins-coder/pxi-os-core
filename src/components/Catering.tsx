   ...
   const handleGroundPrices = async () => {
      ...
      try {
         ...
         Object.entries(groundedPriceMap).forEach(([ingName, price]) => {
            ...
            if (ing) {
               // Note: direct mutation of state-derived objects is normally bad in Zustand, 
               // but here we are using it for a local refresh before a potential store update
               // Actually we should probably have an action for this.
               // For now, I'll keep it simple as the original code did.
            }
         });
         refreshCosting();
      } catch (e) {
...
const OrderBrochure = ({ onComplete, onFinalize, initialEvent }: { onComplete: () => void, onFinalize: (inv: Invoice) => void, initialEvent?: CateringEvent }) => {
   ...
   const [contactEmail, setContactEmail] = useState(initialEvent?.banquetDetails?.contactEmail || '');
      ...
      */
      ...
      try {
         if (initialEvent) {
            ...
            onComplete(); // Close brochure after edit
         } else {
   ...
   return (
      <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-[3rem] shadow-2xl w-full max-w-7xl h-[95vh] overflow-hidden flex flex-col animate-in zoom-in duration-300 border border-slate-200">
         <div className="p-6 md:p-8 border-b-2 border-slate-100 flex flex-col md:flex-row gap-6 justify-between items-center bg-slate-50/50">
            <div className="flex items-center gap-4 shrink-0">
               <div className="w-12 h-12 bg-slate-900 rounded-2xl flex items-center justify-center text-white shadow-lg"><ShoppingBag size={24} /></div>
               <div><h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Banquet Creation</h2><p className="text-[9px] text-slate-500 font-black uppercase mt-1">New Event Node Hub</p></div>
            </div>

            <div className="flex-1 flex justify-center overflow-x-auto max-w-full no-scrollbar px-4">
            ...
            <button onClick={onComplete} className="p-3 bg-white border-2 border-slate-100 hover:bg-rose-500 hover:text-white rounded-2xl transition-all shadow-sm shrink-0"><X size={24} /></button>
         </div>

         <div className="flex-1 flex overflow-hidden">
            <div className="w-[440px] border-r-2 border-slate-100 p-10 space-y-8 bg-slate-50/80 overflow-y-auto scrollbar-thin">
               ...
               <section className="space-y-6">
                  <div className="flex items-center gap-3 border-b-2 border-slate-200 pb-2">
                     ...
                     <h3 className="text-[10px] font-black uppercase text-indigo-600 tracking-widest">Event Planner Detail</h3>
                  </div>
                  <div>
                     <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mb-2">Planner Name</label>
                     <input list="contacts-list-b" className="w-full p-4 bg-white border-2 border-slate-200 rounded-2xl font-black text-slate-950 outline-none focus:border-indigo-500 shadow-sm" placeholder="Search or Enter Name" value={eventPlannerName} onChange={e => setEventPlannerName(e.target.value)} />
                  </div>
                  <div>
                     <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mb-2">Planner Phone</label>
                     <input className="w-full p-4 bg-white border-2 border-slate-200 rounded-2xl font-bold text-xs text-slate-950 outline-none shadow-sm" value={eventPlannerPhone} onChange={e => setEventPlannerPhone(e.target.value)} />
                  </div>
               </section>
               ...
               <section className="space-y-6">
                  <div className="flex items-center gap-3 border-b-2 border-slate-200 pb-2">
                     ...
                     <h3 className="text-[10px] font-black uppercase text-indigo-600 tracking-widest">Additional Notes</h3>
                  </div>
                  <textarea className="w-full p-4 bg-white border-2 border-slate-200 rounded-2xl font-bold text-xs text-slate-950 outline-none resize-none shadow-sm placeholder:text-slate-400" rows={3} placeholder="Dietary restrictions, logistics notes..." value={notes} onChange={e => setNotes(e.target.value)} />
               </section>
            </div>
            ...
            <div className="flex-1 flex flex-col bg-white relative">
                                 ...
                                 {items.map(item => {
                                    ...
                                    return (
                                       <div
                                          ...
                                          className={`group bg-white rounded-[3.5rem] border-2 transition-all overflow-hidden flex flex-col h-full cursor-pointer ${isSelected ? 'border-indigo-600 shadow-2xl ring-4 ring-indigo-50' : 'border-slate-100 shadow-sm'}`}
                                       >
                                          <div className="h-44 w-full relative overflow-hidden bg-slate-50">
                                             ...
                                             {isSelected && <div className="absolute top-4 right-4 w-8 h-8 bg-indigo-600 rounded-full flex items-center justify-center text-white shadow-xl animate-in zoom-in"><Check size={16} /></div>}
                                          </div>
                                          <div className="p-6 flex-1 flex flex-col">
                                             <h4 className="text-lg font-black uppercase tracking-tight text-slate-900 mb-4 leading-tight">{item.name}</h4>
                                             <p className="text-[10px] text-slate-400 font-bold uppercase mb-4 leading-relaxed">{item.description}</p>
               ...
               {/* Itemized Basket Overlay */}
               {isBasketOpen && (
                  <div className="absolute inset-x-0 bottom-0 z-50 bg-slate-950 text-white p-10 border-t-2 border-[#00ff9d]/20 animate-in slide-in-from-bottom-full duration-500 max-h-[70%] overflow-y-auto rounded-t-[4rem]">
                     <div className="flex justify-between items-center mb-10">
                        <div className="flex items-center gap-4">
                           ...
                           <h3 className="text-xl font-black uppercase tracking-tighter">Event Basket</h3>
                        </div>
         ...
         <div className="p-8 border-t-2 border-slate-100 flex justify-between items-center bg-white shadow-2xl relative z-20">
            <div className="flex items-center gap-10">
               <button onClick={() => setIsBasketOpen(!isBasketOpen)} className="flex items-center gap-4 group">
                  <div className="w-14 h-14 rounded-2xl bg-slate-900 flex items-center justify-center text-[#00ff9d] shadow-xl group-hover:scale-110 transition-transform">
                     <ShoppingCart size={24} />
                  </div>
                  <div className="text-left">
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Basket Contents</p>
                     <p className="text-sm font-black text-slate-950 uppercase">{Object.values(selected).filter(q => Number(q) > 0).length} Unique Items</p>
                  </div>
               </button>
               <div className="h-12 w-px bg-slate-100 mx-2"></div>
               <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Aggregate Valuation</p>
                  <span className="text-4xl font-black text-slate-950">₦{(projections.totalRevenue / 100).toLocaleString()}</span>
               </div>
            </div>
            <div className="flex gap-4">
               <button onClick={onComplete} className="px-10 py-5 font-black uppercase text-[10px] text-slate-400">Abort</button>
               <button
                  onClick={handlePlaceOrder}
                  disabled={!hasSelection || isSubmitting || !customerName}
                  className={`px-16 py-6 rounded-[2rem] font-black uppercase text-xs shadow-xl transition-all flex items-center gap-4 ${hasSelection && customerName ? 'bg-slate-950 text-white hover:bg-slate-800' : 'bg-slate-100 text-slate-300 cursor-not-allowed'}`}
               >
                  {isSubmitting ? <RefreshCw className="animate-spin" size={20} /> : <ArrowRight size={20} />} {isPortionLocked ? 'Finalize & Commit' : 'Selection Incomplete'}
               </button>
            </div>
         </div>
      </div>
